...
//בעיה 5- החזרת מסלול עם מינימום פניות:
// אפשרות ב'- אינדוקציה
public class cheapRouteMinTurn {

    public static void main(String[] args) {
        Node[][] mat = InitMat1();
        String minTurn = minTurn(mat);
    ...
    // O((N+M)*CountOfPath)
    private static String minTurn(Node[][] mat) {
        int n = mat.length;
        int m = mat[0].length;
        ...
        String minTurn = getMinTurn(all);
        return minTurn;
    }

    private static LinkedList<String> getAllPaths(Node[][] mat, int i, int j, String
            from, LinkedList<String> lists) {
        if (i > 0 && j > 0) {
    ...
    private static String getMinTurn(LinkedList<String> all) {
        int min = Integer.MAX_VALUE;
        String minTemp = "";
        for (String i : all) {
            int count = 0;
            for (int j = 0; j < i.length() - 1; j++) {
                if (i.charAt(j) == 'r' && i.charAt(j + 1) == 'd') {
                    count++;
                }
                if (i.charAt(j) == 'd' && i.charAt(j + 1) == 'r') {
                    count++;
                }
            }
            if (count < min) {
